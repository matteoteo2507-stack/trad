---
titolo: "TradingView Indicator Builder — prompt onboarding (Free Plan Unlocked all-in-one)"
fonti:
  - "Prompt fornito dall'utente 2026-06-07 (origine esterna)"
tipo: blueprint
stato: archiviato-in-attesa-di-strategia
---

# TradingView Indicator Builder — prompt onboarding

> **STATO: archiviato, NON avviare.** Questo è un prompt operativo da NON eseguire ora.
> Va richiamato **quando avremo una/le strategia/e pronta/e**, oppure quando l'utente
> chiede esplicitamente di configurare i suoi indicatori/setup TradingView. Vedi
> `../../DECISIONS.md` e la roadmap priorità ([[project_roadmap_priority_2026_05_30]]).

## Cosa è (in una riga)

Un prompt conversazionale che costruisce **un singolo indicatore Pine v6 "all-in-one"**
("Free Plan Unlocked") che replica il *valore analitico* di feature TradingView a
pagamento dentro **un solo slot indicatore**, così gira sul **piano Free**. Modalità
Curated (scegli i moduli) o MAX (tutto, al limite fisico di Pine: matrice 21 indicatori ×
6 timeframe). Opzionalmente compila e auto-corregge lo script via un MCP TradingView.

## Perché ci interessa / quando richiamarlo

L'utente lo vuole per **convalidare visivamente su TradingView ciò che la futura strategia
automatica calcola**. Cioè: la strategia gira in automatico (es. detector di
[[project_confluence_auto]] — S/R, S/D, POC), e questo indicatore disegna gli stessi
elementi a schermo per un controllo a occhio (eyeball check) prima/durante il trading.

**Mappatura moduli del prompt → cosa calcoliamo noi** (da rifinire quando la strategia è
definita):
- **Key Levels (PDH/PDL/PWH/PWL)** ≈ parte dei livelli S/R orizzontali del detector.
- **Volume Profile (approx.) / POC** ≈ il detector POC (volume profile rolling). NB: il
  Pine usa hlc3×volume come approssimazione, non profilo intrabar reale — buono per
  vedere i nodi ad alto volume, non identico al nostro POC su tick_volume MT5.
- **Higher-Timeframe Levels** ≈ filtro per timeframe (TRADING_PRINCIPLES §2: TF più alto =
  livello più forte).
- **Signal Markers (EMA fast/slow + RSI)** ≈ marker di entry; da riallineare ai trigger
  *veri* della strategia (oggi è un placeholder EMA-cross, NON la nostra logica).
- **MTF Dashboard / Screener** = contesto multi-TF e multi-simbolo.
- **Sessions / Kill Zones** = sessioni Asia/London/NY (utile per le strategie session-based).

**Allineamento da fare al richiamo:** i default del prompt (EMA 9/21, RSI 14, ecc.) sono
generici. Quando la strategia sarà pronta, vanno **sostituiti con i parametri/logica reali**
della strategia, altrimenti l'indicatore "valida" qualcosa di diverso da ciò che gira in
automatico. Lo scopo è far coincidere il visuale col calcolato, non avere due logiche.

## Caveat tecnici / contesto memoria

- **MCP diverso da quello già valutato.** Questo prompt usa
  `LewisWJackson/tradingview-mcp-jackson` (compila Pine sul TradingView Desktop, piano Free
  ok). È COSA DIVERSA dal `tradesdontlie/tradingview-mcp` valutato e accantonato
  ([[reference_tradingview_mcp]]). Limite comune che resta valido: **nessun MCP TradingView
  legge i drawings manuali della toolbar** — leggono/scrivono solo oggetti Pine. Quindi
  questo serve a *disegnare via Pine* ciò che la strategia calcola, NON a *leggere* i
  livelli che l'utente ha tracciato a mano.
- Limiti onesti NON aggirabili in Pine (lo dice il prompt stesso): bar replay intraday,
  chart al secondo/tick, più storico, rimozione ad, volume profile intrabar vero.
- Setup MCP richiede TradingView Desktop installato + account loggato (Free basta) e
  **riavvio di Claude Code** dopo l'install (gli MCP si caricano all'avvio). Se non si
  vuole l'MCP, lo script si costruisce comunque ma senza auto-compile/auto-fix.

## Come richiamarlo

Quando la strategia è pronta: ripartire da questo file, **NON ri-eseguire i default a
scatola chiusa** — prima decidere quali moduli mappano i componenti della strategia e con
quali parametri, poi seguire il flusso Step 0–5 del prompt.

> ⚠️ **NB sul testo qui sotto:** il prompt originale incollato dall'utente il 2026-06-07 è
> stato **troncato dall'ambiente** (limite ~50k caratteri) dentro il "MAX BASE SCRIPT",
> nella funzione `f_scan` della matrice 21×6. Il **BASE SCRIPT curato è COMPLETO**; il
> **MAX BASE SCRIPT è INCOMPLETO**. Al richiamo, se serve la modalità MAX, farsi
> **re-incollare il MAX BASE SCRIPT integrale** dall'utente. Il punto di taglio è marcato
> sotto con `[[[TRONCATO QUI — MAX BASE SCRIPT INCOMPLETO]]]`.

---

# PROMPT ORIGINALE (verbatim, da NON eseguire — vedi nota di troncamento)
...
[[[ TRONCATO QUI — MAX BASE SCRIPT INCOMPLETO ]]]
Il prompt originale incollato dall'utente è stato tagliato dall'ambiente a questo
punto (dentro la funzione f_scan della matrice 21x6). Il MAX BASE SCRIPT sotto questo
header NON è completo. Per usare la modalità MAX, farsi re-incollare dall'utente il
MAX BASE SCRIPT integrale. Frammento ricevuto fino al taglio (parziale, NON usare
così com'è):

//@version=6
// FREE PLAN UNLOCKED - MAX (everything, pushed to Pine's hard ceiling)
// 21-indicator x 6-timeframe matrix (126/127 tuple elements, 6/40 security calls)
// + full overlay suite computed locally. [resto identico al BASE SCRIPT per gli
//   overlay: MA suite, VWAP/Anchored VWAP, Key Levels via running PDH/PDL/PWH/PWL,
//   Sessions, Signals] poi la MATRICE:
//   f_scan(tf) => request.security(syminfo.tickerid, tf, [ema9>ema21, ema21>ema50,
//   ema50>ema200, rsi14, stoch14, macdBull, macdHist, cci20, mfi14, adx, diBull,
//   bbPctB, wpr14, roc9, atr%, obv>obv[1], stBull, mom10>0, ...]) su 6 TF
//   (default 5/15/60/240/D/W). [INTERROTTO]
```
